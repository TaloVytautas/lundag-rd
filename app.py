...
if st.button("Start Scraping", type="primary"):
    ...
    # Format the dates perfectly for the WordPress API (ISO 8601 format)
    start_iso = f"{start_date.strftime('%Y-%m-%d')}T00:00:00"
    end_iso = f"{end_date.strftime('%Y-%m-%d')}T23:59:59"

    with st.spinner("Fetching batches of 100 articles..."):
        while True:
            status_text.text(f"Fetching page {page}...")
            
            # Request articles strictly within our date range
            api_url = f"https://www.lundagard.se/wp-json/wp/v2/posts?per_page=100&page={page}&after={start_iso}&before={end_iso}"
